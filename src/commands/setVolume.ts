...
const setVolume: CommandOptions = {
    ...
    execute: async (message: Message, args: string[])=>{
        ...
        if (!member?.voice.channel)
                return await message.channel.send('You have to join a voice channel in order to do this').then((msg)=> msg.delete({timeout: 4000}));
        ...
        if(!(parseInt(args[0])>=0 && parseInt(args[0])<=1)) {
            return await message.channel.send("You have to use number bigger than 0 and smoller than 1 (for example 0.4)").then((msg)=> msg.delete({timeout: 4000}));
        }
