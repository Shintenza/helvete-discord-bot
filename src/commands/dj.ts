...
const dj: CommandOptions = {
    ...
    execute: async(message, args)=>{
        ...
        if(! messageAuthorMember.permissions.has("MANAGE_ROLES")){
            return await message.channel.send('You are not allowed to do this').then((msg)=> setTimeout(()=>msg.delete(),4000));
        }
        const target = args[0];
        ...
        if(!role) {
            return await message.channel.send('Dj role has not been found').then((msg)=> setTimeout(()=>msg.delete(),4000));
        }
        await targetMember.roles.add(role)
            .then(async ()=>{
                return await message.channel.send('Dj role has been given to the user').then((msg)=> setTimeout(()=>msg.delete(),4000));
            })
            .catch(async err=> await message.channel.send('Something went wrong').then((msg)=> setTimeout(()=>msg.delete(),4000)));
        return
    }
}
export = dj;