...
const remove: CommandOptions = {
    ...
    execute: async(message: Message, args)=>{
        ...
        if(args[0]=="0") {
            return await message.channel.send("Wrong number of track").then((msg)=> setTimeout(()=>msg.delete(),4000));
        }
        if (!serverQueue || serverQueue.queue.length <= 1) {
            return await message.channel.send("There is nothing to remove").then((msg)=> setTimeout(()=>msg.delete(),4000));
        }
        ...
        if(!voiceConnection || !voiceConnection.connection?.dispatcher ) {
            return await message.channel.send("There is nothing playing right now").then((msg)=> setTimeout(()=>msg.delete(),4000));
        }
